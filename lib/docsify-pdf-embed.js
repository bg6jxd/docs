...
const PDF_VIEWER_HEIGHT = '45rem';
const PDF_MARGIN_TOP = '2rem';
const PDF_MARGIN_BOTTOM = '2rem';
	...
	window.$docsify.plugins = [
		function(hook, vm) {
			...
			hook.afterEach(function(html, next) {
				...
				if(container_list){
					// Handling mobile pdf rendering  https://mozilla.github.io/pdf.js/web/viewer.html
					if(mobile_view){
						html += '<script>';
						container_list.forEach(function(container){
							...
							var view_location = "https://qumoptly.github.io/pdf.js/es5/web/viewer.html?file=" + absolute_pdf_location;
						...
						// Handling desktop pdf rendering  https://qumoptly.github.io/pdfh5/pdf.html
						/* PDFObject
						html += '<script>';
						container_list.forEach(function(container){
							html += '\
						...
						container_list.forEach(function(container){
							var absolute_pdf_location = url_check(container['pdf_location']) ? container['pdf_location'] : build_absolute_path(container['pdf_location']);
							var view_location = "https://qumoptly.github.io/pdf.js/web/viewer.html?file=" + absolute_pdf_location;
							var iframe_id = container['div_id'].concat('-iframe');
							html += '\
									document.getElementById("'+ container['div_id'] +'").innerHTML = \
									\'<iframe \
